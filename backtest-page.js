...
var METHODS = {
  ...
  letf:   { label:'Leveraged ETF', spread:1.0,
...
var UI = { start:null, rebal:'annual', lev:2, spread:1.5, asset:'mix', method:'margin' };
...
window.setUI = function (k, v) {
  ...
  if (k === 'lev' || k === 'spread' || k === 'asset' || k === 'method') renderLev();
  else renderHist();
};
...
function renderLev() {
  ...
  for (i = from; i <= to; i++) {
    ...
    for (j = 0; j < ks.length; j++) if (S[ks[j]][i] == null) { skip = true; break; }
    if (skip || S.CASH[i] == null) continue;
    var rr = 0; for (j = 0; j < ks.length; j++) rr += w[ks[j]] * S[ks[j]][i];
    xs.push(rr - S.CASH[i]);
  }
  ...
  /* every method at the chosen leverage, so the trade-off is visible */
  ...
  el.innerHTML =
    ...
    '</div>' +
    '<div class="note" style="margin-bottom:18px"><b>' + METHODS[UI.method].label + '.</b> ' + METHODS[UI.method].blurb + '</div>' +
    '<div class="ctlgrid">' +
      ...
      '<div class="field"><span class="lbl">Interest above T-bills</span>' +
        '<div style="display:flex;align-items:center;gap:12px">' +
          '<input type="range" min="0" max="5" step="0.25" value="' + UI.spread + '" oninput="setUI(&#39;spread&#39;,this.value)" style="width:150px">' +
          '<span class="readout num">' + UI.spread.toFixed(2) + '%</span></div></div>' +
      '<div class="field"><span class="lbl">Period</span><span style="font-size:14px;color:var(--muted);padding-top:5px">' + M[from].slice(0,4) + '&ndash;' + M[to].slice(0,4) + ', ' + Math.round(base.n / 12) + ' years</span></div>' +
    '</div>' +
    '<div class="strip">' +
    ...
    '</div>' +
    '<div class="card"><div class="big-title">What is &ldquo;interest above T-bills&rdquo;?</div>' +
      ...
      '<table style="margin-bottom:14px">' +
        ...
        '<tr><td><b>1&ndash;1.5%</b></td><td style="text-align:left" class="dim">Leveraged funds. <b>SSO</b> is 2x the S&amp;P 500, <b>UPRO</b> is 3x, <b>TQQQ</b> is 3x the Nasdaq.</td></tr>' +
      ...
      '<div class="danger"><b>The leveraged ETF model is still optimistic.</b> Real funds reset every day; this estimates that decay from monthly data. In a genuinely violent month the real thing does worse than shown here. Treat leveraged ETF numbers as a best case.</div>' +
    '</div>' +
