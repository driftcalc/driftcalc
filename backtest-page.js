...
var METHODS = {
  margin: { label:'Margin', spread:1.5,
    ...
    noun:'margin',
    range:'0.5&ndash;1.5% on portfolio margin, 2&ndash;5% on the ordinary kind',
    cost:'About 0.5&ndash;1.5% above T-bills if you qualify for portfolio margin, 2&ndash;5% on the ordinary kind most brokers give you. Nothing else on top &mdash; you hold real shares, so you keep the dividends.' },
  letf:   { label:'Leveraged ETF', spread:0.6,
    ...
    noun:'a leveraged ETF',
    range:'0.4&ndash;0.8%',
    cost:'About 0.4&ndash;0.8% above T-bills, which is what the fund itself pays on swaps and futures. On top of that a 0.90% expense ratio charged on the <i>whole</i> position rather than just the borrowed part &mdash; that is the bigger number. Both are already in the results.' },
  leaps:  { label:'LEAPS options', spread:1.0,
    ...
    noun:'LEAPS',
    range:'0.5&ndash;1.5%',
    cost:'About 0.5&ndash;1.5% above T-bills, baked into the option price rather than quoted anywhere, plus a few tenths in bid-ask every time you roll. On top of that you give up the ~1.8% dividend on the entire position, because you own options and not shares. Both are already in the results.' },
};
...
(function injectCss(){
  ...
  s.textContent =
    ...
    /* Rate table: keep the percentage on one line and off the prose beside it. */
    '.ratetbl{table-layout:auto}' +
    '.ratetbl td:first-child,.ratetbl th:first-child{width:23%;min-width:150px;vertical-align:top;padding-right:16px}' +
    '.ratetbl td:nth-child(2),.ratetbl th:nth-child(2){white-space:nowrap;width:1%;vertical-align:top;' +
      'padding-left:8px;padding-right:22px;text-align:right}' +
    '.ratetbl td:nth-child(3){vertical-align:top;line-height:1.55}' +
    '.ratetbl .sub{display:block;font-weight:400;font-size:12.5px;color:var(--faint);margin-top:2px}' +
    '@media(max-width:640px){.ratetbl td:first-child,.ratetbl th:first-child{width:auto;min-width:0}' +
      '.ratetbl td:nth-child(2){padding-right:10px}}' +
...
function renderLev() {
  ...
  if (wiped) {
    ...
    verdict = '<b>Over the last ' + actualYears + ' years, ' + UI.lev.toFixed(1) + 'x with ' +
      METHODS[UI.method].noun + ' earned ' + pct(Math.abs(delta)) + ' a year ' + (delta >= 0 ? 'more' : 'less') +
  ...
  el.innerHTML =
    ...
    '<div class="note" style="margin-bottom:18px"><b>' + METHODS[UI.method].label + '.</b> ' + METHODS[UI.method].blurb +
      '<br><span style="color:var(--faint)"><b>What it costs you.</b> ' + METHODS[UI.method].cost + '</span></div>' +
    '<div class="ctlgrid">' +
      ...
      '<div class="field"><span class="lbl">Interest above T-bills</span>' +
        ...
        '<span style="font-size:12px;color:var(--faint);line-height:1.5;max-width:250px;display:block">' +
          'Typical for ' + METHODS[UI.method].noun +
    ...
    '<details class="acc"><summary>What is &ldquo;interest above T-bills&rdquo;?<span class="hint">and which fund you would actually buy</span></summary><div class="accbody">' +
      ...
      '<table class="ratetbl" style="margin-bottom:14px">' +
        '<tr><th style="text-align:left">How you borrow</th><th style="text-align:right">Over T-bills</th><th style="text-align:left">And what else it costs</th></tr>' +
        '<tr><td><b>Futures</b><span class="sub">/ES, or /MES at a fifth the size</span></td>' +
        ...
        '<tr><td><b>Leveraged ETF</b><span class="sub">SSO 2x, UPRO 3x, TQQQ 3x Nasdaq</span></td>' +
        ...
        '<tr><td><b>LEAPS</b><span class="sub">deep in-the-money calls, 1&ndash;2 years out</span></td>' +
          '<td class="num"><b>0.5&ndash;1.5%</b></td>' +
