/* Drift Calc — backtest and leverage lab.
   Reads the allocation set on the calculator page (same localStorage key, same
   origin) and returns.json, which the GitHub Action rebuilds monthly. */
(function () {
'use strict';
...
/* Constant leverage, reset monthly, borrowing the levered part at cash + spread.
   Same mechanic as a levered fund, at monthly rather than daily resolution.
   Because leverage resets every month the sleeve weights are always exactly w,
   so the blended asset return is a plain weighted sum. */
...
/* Unlevered path, with real drift between rebalances. */
...
function renderLev() {
  ...
  var cw = 640, ch = 210, pad = 34, topPad = 40;
  ...
  var chart = '<svg viewBox="0 0 ' + cw + ' ' + ch + '" style="width:100%;height:210px;display:block">' +
    ...
    '<text x="' + X(bestPt.L) + '" y="' + (Y(bestPt.g) - 12) + '" font-size="11" font-weight="600" fill="#17A374" text-anchor="middle">best was ' + bestPt.L + 'x</text>' +
    (here.g > -1 ? '<circle cx="' + X(here.L) + '" cy="' + Y(here.g) + '" r="4.5" fill="var(--ink)"/>' : '') +
    '</svg><div style="font-size:11.5px;color:var(--faint)">Return per year at each leverage level, after borrowing costs. The black dot is where your slider is. Past the green peak, more borrowing earned less money.</div>';

  var wiped = lev.wiped;
  var breakeven = (100 / UI.lev).toFixed(0);
  el.innerHTML =
    '<div class="note" style="margin-bottom:18px">' +
      '<b>What this page is.</b> Leverage means borrowing money so you own more than you paid for. ' +
      'At 2x, every $1 of yours holds $2 of stocks and owes $1. Gains double. So do losses. ' +
      'This runs that through real history and shows where it stops being worth it.' +
    '</div>' +
    '<div class="ctlbar">' +
    ...
    '</div>' +
    '<div class="strip">' +
      '<div class="stat"><div class="k">No borrowing</div><div class="v num">' + pct(cagr(base.val, base.n)) + '</div><div class="s">a year</div></div>' +
      '<div class="stat"><div class="k">At ' + UI.lev.toFixed(1) + 'x</div><div class="v num ' + (wiped ? 'down' : (cagr(lev.val, lev.n) > cagr(base.val, base.n) ? 'up' : 'down')) + '">' + (wiped ? 'wiped out' : pct(cagr(lev.val, lev.n))) + '</div><div class="s">a year, after interest</div></div>' +
      ...
      '<div class="stat"><div class="k">Worst month</div><div class="v num down">' + pct(lev.worstMonth) + '</div><div class="s">-' + breakeven + '% would end it</div></div>' +
    '</div>' +
    (wiped ? '<div class="danger" style="margin-bottom:16px"><b>Gone in ' + wiped.slice(0, 4) + '.</b> At ' + UI.lev.toFixed(1) + 'x, one month down more than ' + breakeven + '% takes everything. There is no recovering from zero, and no amount of being right afterwards fixes it.</div>' : '') +
    '<div class="card">' + chart + '</div>' +

    '<div class="card"><div class="big-title">What is &ldquo;borrow cost over T-bills&rdquo;?</div>' +
      '<div class="note" style="margin-bottom:14px">' +
        'To use leverage you borrow money, and borrowing is not free. That slider is the interest rate you pay.<br><br>' +
        '<b>T-bills</b> are short-term US government IOUs, the cheapest borrowing rate that exists, because the government is the safest borrower there is. ' +
        'Nobody lends to you at that rate. The slider is how many percentage points <i>above</i> it you pay. ' +
        'So at 1.00%, if T-bills pay 4%, you are borrowing at about 5%.' +
      '</div>' +
      '<table style="margin-bottom:14px">' +
        '<tr><th style="text-align:left">You pay</th><th style="text-align:left">How you would actually do it</th></tr>' +
        '<tr><td><b>0.1&ndash;0.5%</b></td><td style="text-align:left" class="dim">S&amp;P 500 futures (<b>/ES</b>, or <b>/MES</b> at a fifth the size), or a box spread on SPX options. The cheapest money a retail account can get. Both need options approval and neither is a beginner move.</td></tr>' +
        '<tr><td><b>0.5&ndash;1.5%</b></td><td style="text-align:left" class="dim">Portfolio margin, usually Interactive Brokers. Needs a decent account size to qualify.</td></tr>' +
        '<tr><td><b>1&ndash;1.5%</b></td><td style="text-align:left" class="dim">Leveraged funds. <b>SSO</b> is 2x the S&amp;P 500, <b>UPRO</b> is 3x, <b>TQQQ</b> is 3x the Nasdaq. Roughly a 0.9% expense ratio plus the fund&rsquo;s own borrowing. No margin account needed. Read the red box below.</td></tr>' +
        '<tr><td><b>2&ndash;5%</b></td><td style="text-align:left" class="dim">Ordinary margin at Schwab, Fidelity, E*TRADE, Robinhood. What most people can actually get, and where the math stops working. Drag the slider up there and watch the peak collapse toward 1x.</td></tr>' +
      '</table>' +
      '<div class="warn" style="margin-bottom:12px"><b>Look up your own number.</b> Every broker publishes its margin rate and it moves with the Fed. Those ranges are ballparks, not quotes. If you cannot find yours, assume it is worse than you hoped.</div>' +
      '<div class="danger"><b>Leveraged funds reset daily. This model resets monthly.</b> SSO, UPRO and TQQQ rebalance their borrowing every single day, so in a choppy market they bleed value even when the index finishes flat. Two down days and two up days of the same size leave you behind. This page does not model that, which means <b>it is too kind to those funds</b>. Treat anything it shows for them as a best case.</div>' +
    '</div>' +
    ...
    '<div class="card"><div class="big-title">What the math says</div>' +
      '<table>' +
        ...
        '<tr><td>What worked best in this exact history</td><td class="num">' + (bestPt.L >= 3 ? '3x or more' : bestPt.L + 'x') + '</td></tr>' +
      '</table>' +
      '<div class="note" style="margin-top:14px"><b>Why there is a peak at all.</b> Leverage multiplies your return but squares your drag. Compounded growth runs roughly <code>L(&mu;&minus;r) + r &minus; L&sup2;&sigma;&sup2;/2</code>: the gain grows in a straight line, the penalty grows as a square. Past a point, more borrowing means less money. That turning point is Kelly.</div>' +
      (kelly > 3 ? '<div class="danger" style="margin-top:12px"><b>Ignore that number.</b> Kelly divides by variance, so anything calm returns a huge answer &mdash; a bond-heavy mix can print 8x or 10x. Low measured volatility is not low risk, it usually means the bad day has not happened yet in this window. Levering quiet assets is how Long-Term Capital Management died in 1998 and how risk parity funds got hurt in 2020 and 2022.</div>' : '') +
    '</div>' +
    ...
    '<div class="card"><div class="big-title">Why that is not a recommendation</div>' +
      ...
      '<div class="warn" style="margin-bottom:12px"><b>Borrowing cost decides everything.</b> Move the slider. Futures sit near T-bills; ordinary margin runs 3 to 5 points above, and at that price leverage stops working almost immediately.</div>' +
